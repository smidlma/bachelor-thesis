...
def test_local_transformation():
    sortT = transformation.Sort(
        name='Sort', position=0, columns=['first_name'])
    sortT2 = transformation.Sort(
        name='Sort', position=1, columns=['ip_address'])
...
def test_global_transformations():
    pip = Pipeline('My pip')
    sortT = transformation.Sort(
        name='Sort', position=0, columns=['first_name']).save()
    sortT2 = transformation.Sort(
        name='Sort', position=1, columns=['ip_address']).save()
    csv = source.CSV('csv', 'mock.csv').save()
    csv2 = source.CSV('csv2', 'mock.csv').save()
    csv.addTransformation(sortT)
    csv2.addTransformation(sortT2)
    join = source.Join(
        name='Join',s1=csv, s2=csv2, on='id', how='left').save()
    pip.addJoin(join)
    pip.addSource(csv)
    pip.addSource(csv2)
    pip.save()
